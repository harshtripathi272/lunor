...
app = FastAPI(title="Lunor Translator  API", version="1.0.0")
...
if os.getenv("TAURI_ENV") or "tauri" in os.getcwd().lower():
...
async def text_to_speech(request: TTSRequest):
    """Cross-platform offline text-to-speech for English and Hindi"""
    ...
    import platform, subprocess, wave, numpy as np
    ...
    def system_tts():
        try:
            ...
            logger.info(f"🌐 Running system TTS fallback on {system}")
            ...
            if system == "windows":
                ...
                result = subprocess.run(["say", "-o", audio_path, "--data-format=LEF32@22050", request.text],
                                        capture_output=True, timeout=10)
                return result.returncode == 0

            elif system == "linux":
                result = subprocess.run(["espeak", "-w", audio_path, request.text],
                                        capture_output=True, timeout=10)
                return result.returncode == 0
    ...
    # Run TTS
    success = await asyncio.get_event_loop().run_in_executor(None, system_tts)

    # Wait up to 5s for file flush
    max_wait_time = 5.0
    poll_interval = 0.1
